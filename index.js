...
const {  state, saveCreds } =await useMultiFileAuthState(`./sessions`)
   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
    ...
    if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
        ...
        phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number again 😍\nFor example: +916909137213 : `)))
        phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
    }
    
    // If phone number is valid, proceed with pairing
    setTimeout(async () => {
        if (phoneNumber) {
            ...
            console.log(chalk.bgRed("Invalid phone number entered, please check and try again"))
            process.exit(0)
        }
    }, 3000)
}
